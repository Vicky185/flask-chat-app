...
def user(username):
    ...
    #then we call our add_messages function with the two variables we've just created, username and message
    #then we return redirect(session[username]). what we're doing  is just obtaining our username and our message variables. then  we're going to send those both into our add_messages function to add them to the list.
    if request.method == "POST":
